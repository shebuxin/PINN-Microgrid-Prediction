...
\begin{itemize}
  \item Symmetric for reciprocal branches without phase-shifting effects: $Y_{ij}=Y_{ji}$.
...
These equations are \emph{nonlinear} in $|V|$ and $\theta$ --- products of voltage terms and trig functions. There is no general closed-form solution for an arbitrary network, so practical solvers iterate.
\end{alertblock}
\end{frame}
...
``Decoupled PF'' and ``fast-decoupled PF'' approximate $J$ by neglecting weak coupling blocks. They can reduce work per iteration, but convergence depends strongly on the network; pandapower's default balanced AC solver is Newton--Raphson.
\end{alertblock}
\end{frame}
...
\begin{enumerate}
  \item \warnitem \textbf{Non-convergence.} The solver hits the iteration limit. Common causes include a missing slack, islanding, bad input data, or an operating point near voltage collapse. A thermal rating itself is not part of the PF equations.
...
\node[dev, below=0.6cm of b2, fill=red!10] {Load\\$P=0.45, Q=0.18$};
\node[dev, below=0.6cm of b3, fill=red!10] {Load\\$P=0.30, Q=0.12$};
...
\end{enumerate}
\vspace{0.5em}
\begin{block}{Numerical result (converged)}
$|V_2| \approx 0.964, \theta_2 \approx -2.3^\circ, |V_3| \approx 0.944, \theta_3 \approx -3.6^\circ$. Converges in 3 iterations. You can reproduce this with \code{pp.runpp(net)} on a 100 MVA, 20 kV system.
\end{block}
\end{frame}
