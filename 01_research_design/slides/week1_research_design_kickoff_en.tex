...
% ============================================================
\begin{frame}[shrink=7]{Project-driven: working backward from the paper}
...
\tikzset{box/.style={draw, rounded corners, align=center, minimum width=2.2cm, minimum height=0.75cm, fill=blue!6}}
...
\end{itemize}
\vspace{0.4em}
\textbf{macOS / Linux (official automatic installer):}
\begin{lstlisting}
...
\end{lstlisting}
\vspace{0.3em}
The versioned file installs the stack used to generate the reference outputs:
\begin{itemize}
  \item Python 3.11 and \code{pandapower=3.2.1};
  \item NumPy 1.26.4, pandas 2.2.3, and SciPy 1.13.1;
...
\end{itemize}
Using the checked-in file avoids silent dependency drift between students.
\end{frame}
...
# 2.10.x
python -m ipykernel install --user --name pdpower \
  --display-name "Python (pdpower)"
\end{lstlisting}
\vspace{0.4em}
...
\begin{enumerate}
  ...
  \item In the notebook kernel picker, choose \textbf{Python (pdpower)}.
  \item If it is missing, rerun the \code{ipykernel install} command from Step 4 and reload VSCode.
...
\begin{enumerate}
  ...
  \item \warnitem Installing a different pandapower release and comparing its output byte-for-byte with the reference notebooks. Use \code{environment.yml} for reproducibility.
  \item \warnitem Looking only in \textbf{Python: Select Interpreter}. VSCode's Jupyter kernel picker uses a separate discovery path, so register the \code{pdpower} kernel explicitly.
