...
\begin{block}{本周边界}
本周先用平衡模型建立仿真素养。Week 2 再进入 \code{runpp\_3ph}, \code{asymmetric\_load}, \code{asymmetric\_sgen} 和三相结果表。
\end{block}
...
\column{0.42\textwidth}
\begin{block}{工程叙事}
传统配电网分析常看单个工况; 微电网需要看大量 operating scenarios 和 contingency。AI 的角色是 fast screening, 不是取代潮流软件。
\end{block}
...
\vspace{0.8em}
\begin{block}{研究问题}
能否用 physics-informed AI 在高召回率前提下减少大量三相 N-1 潮流调用?
\end{block}
\end{frame}
...
\end{table}
\begin{alertblock}{重点}
pandapower storage 文档明确说明: \code{p\_mw} 正值为 charging, 负值为 discharging; SOC 不会在默认潮流计算中自动更新。\footnote{pandapower 3.4.0 storage documentation.}
\end{alertblock}
\end{frame}
...
% ----------------------------------------------------------------------
\begin{frame}{Notebook 的验证哲学}
\begin{block}{我们不接受 ``代码跑通了'' 作为正确性证据}
本周 notebook 对每个关键结果加入 proof / audit / cross-validation。
\end{block}
\begin{enumerate}
  \item \checkitem 网络结构检查: bus, line, transformer 数量; rated voltage; radial connected topology;
  \item \checkitem 潮流收敛检查: \code{net.converged} 必须为 True;
  \item \checkitem 功率平衡检查: 外部电网功率与负荷、PV、BESS、损耗闭合;
  \item \checkitem 线路 loading 手算复核: 用 terminal current 和 current rating 重算 loading;
  \item \checkitem 跨求解器验证: Newton-Raphson 与 BFSW 的电压和 loading 接近;
  \item \checkitem 场景恢复检查: scenario loop 不能污染 base network。
\end{enumerate}
\end{frame}
...
\end{lstlisting}
\begin{block}{为什么要这样做?}
学生必须理解 \code{loading\_percent} 不是神秘输出, 而是可以从电流和额定值复核的工程量。
\end{block}
\end{frame}
...
\begin{enumerate}
  ...
  \item 让学生解释 PV 和 BESS 对 grid import 的影响;
...
\begin{frame}{课堂讨论问题}
\begin{itemize}
  \item 如果只能监视 3 个变量, 你会选择哪些变量作为安全预测特征?
  ...
  \item 为什么 AI screening 的 recall 比 accuracy 更关键?
\end{itemize}
\end{frame}
...
\end{alertblock}
\footnotesize pandapower 文档说明 \code{runpp\_3ph} 用 sequence frame 求解三相潮流, 正序网络使用 NR, 零序和负序网络使用 current injection 方法。\footnote{pandapower 3.4.0 asymmetric / three phase power-flow documentation.}
\end{columns}
\end{frame}

% ----------------------------------------------------------------------
\begin{frame}{本周 exit checklist}
\begin{enumerate}
  \item \checkitem 我能用一句话说明课题目标;
  \item \checkitem 我能解释 microgrid, PCC, PV, BESS, critical load;
  \item \checkitem 我知道 \code{net.*} 输入表和 \code{net.res\_*} 结果表的区别;
  \item \checkitem 我能运行 \code{pp.runpp(net)} 并检查 \code{net.converged};
  \item \checkitem 我能解释 min voltage 和 max loading 的物理意义;
  \item \checkitem 我能说明 storage 正负号约定;
  \item \checkitem 我能用功率平衡和 line loading 公式复核结果;
  \item \checkitem 我知道 Week 2 为什么必须进入三相不平衡潮流。
\end{enumerate}
\end{frame}
...
\begin{itemize}
  ...
  \item pandapower 3.4.0 documentation, \emph{Balanced AC Power Flow}.
  \item pandapower 3.4.0 documentation, \emph{Storage element}.
  \item pandapower 3.4.0 documentation, \emph{Asymmetric / Three Phase Power Flow}.
\end{itemize}
\vspace{0.8em}
\begin{block}{材料使用建议}
本课件与 Notebook 1 配套使用。课堂中重点让学生运行、修改、解释和验证, 而不是只听概念。
\end{block}
\end{frame}
